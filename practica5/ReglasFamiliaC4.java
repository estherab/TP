...
public abstract class ReglasFamiliaC4 implements ReglasJuego {
	...
	protected boolean comprobarHorizontal(Tablero tab, int fila, int col) {
		...
		while ((c < ancho - 1) && (tab.getCasilla(fila, c + 1) == tab.getCasilla(fila, c)) && (coincidentes < 4)) {
			coincidentes++;
			c++;
		}
		c = col; 

		while ((c > 0) && (tab.getCasilla(fila, c - 1) == tab.getCasilla(fila, c)) && (coincidentes < 4)) {
			coincidentes++; 
			c--;
		}
	...
	protected boolean comprobarDiagonalCreciente(Tablero tab, int fila, int col) {
		...
		while ((f < alto - 1) && (c < ancho - 1) && (tab.getCasilla(f + 1, c + 1) == tab.getCasilla(f, c)) && (coincidentes < 4)) {
			coincidentes++;
			f++;
			c++;
		}
		...
		while ((f > 0) && (c > 0) && (tab.getCasilla(f - 1, c - 1) == tab.getCasilla(f, c)) && (coincidentes < 4)) {
			coincidentes++;
			f--;
			c--;
		}
	...
	protected boolean comprobarDiagonalDecreciente(Tablero tab, int fila, int col) {
		...
		while ((f < alto - 1) && (c > 0) && (tab.getCasilla(f + 1, c - 1) == tab.getCasilla(f, c)) && (coincidentes < 4)) {
			coincidentes++;
			f++;
			c--;
		}
		...
		while ((f > 0) && (c < ancho - 1) && (tab.getCasilla(f - 1, c + 1) == tab.getCasilla(f, c)) && (coincidentes < 4)) {
			coincidentes++; 
			f--;
			c++;
		}
