...
public class ReglasGravity extends ReglasFamiliaC4 {
	...
	public void movimientoValido(Tablero tab, Movimiento mov) throws MovimientoInvalido {
		...
		if ((mov.getColumna() < 0) || (mov.getColumna() >= columnas_gr) || (mov.getFila() < 0) || (mov.getFila() >= filas_gr))
			throw new MovimientoInvalido("Posici�n incorrecta. Las dimensiones son de: " + filas_gr + " x " + columnas_gr);
	...
	public boolean tableroLleno(Tablero tab) {
		...
		// Se sale del bucle en cuanto que encuentre una ficha vacia
		while (fil < tab.getAlto() && lleno) {
			col = 0;
			while (col < tab.getAncho() && lleno) {
				if (tab.fichaVacia(fil, col))
					lleno = false;
				else
					col++;
			}
	...
	protected boolean comprobarVertical(Tablero tab, int fila, int col) {
		int coincidentes = 1, f = fila, alto = tab.getAlto();
		
		// Comrpueba hacia arriba
		while ((f < alto - 1) && (tab.getFicha(f + 1, col) == tab.getFicha(f, col)) && (coincidentes < 4)) {
			f++;
			coincidentes++;
		}
			f = fila;
		...
		while ((f > 0) && (tab.getFicha(f - 1, col) == tab.getFicha(f, col)) && (coincidentes < 4)) {
			coincidentes++;
			f--;
		}
